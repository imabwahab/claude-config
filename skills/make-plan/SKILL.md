...
- A link (do not fetch — ask the user to paste the relevant content)
...
5. Note the repo's conventions: naming (snake_case vs camelCase), file layout, branch naming if visible from `git branch -a` or recent commits, commit style.
...
- **Branch convention**: if visible, e.g., `feature/<initials>/<ticket>`; otherwise say "not visible".
...
3. **Affected areas** — grouped by category (backend, data layer, frontend, auth, integrations, infra, tests, docs — only the ones that apply). Each item names concrete paths from the investigator reports. If the ticket touches an area the repo does not currently have, say so explicitly.
4. **Approach** — a numbered walkthrough of how the change flows end-to-end (e.g., user action → frontend handler → API call → backend route → service → data layer → response). A map, not pseudocode.
...
8. **Open questions** — anything the ticket did not specify that the implementer needs answered. Omit if empty.
...
- **Cite real paths** from the investigator reports. Use `path/to/file.ext:line`. No fabricated paths — if the investigator didn't surface it, don't invent it.
...
Save the plan as a markdown file. Default filename:

- If the ticket has an ID: `.claude/plans/<ticket-id>-<short-slug>.md` (e.g., `.claude/plans/PROJ-123-add-csv-export.md`)
- Otherwise: `plans/<short-slug>.md`

Confirm the path with the user if a `./claude/plans/` directory does not already exist, or if the investigator surfaced a different convention (e.g., `docs/plans/`, `.claude/plans/`). Show the user where the file was saved.